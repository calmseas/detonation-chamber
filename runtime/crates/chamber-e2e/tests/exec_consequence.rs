...
impl std::ops::Deref for CellGuard {
    ...
    fn deref(&self) -> &Container {
        self.0.as_ref().expect("container is present until CellGuard is dropped")
    }
}
...
fn substitute_runs_the_replacement_binary() {
    ...
    let plan = ExecConsequencePlan {
        rules: vec![ExecConsequenceRule {
            ...
            verb: ExecVerb::Substitute { replacement_argv: vec!["/usr/bin/curl".to_owned()] },
        }],
        timeout_ms: 60_000,
    ...
    // program did".
    let out = cell.exec(&["stub", "/bin/false", "--version"], OP_WINDOW).expect("exec");
    assert_eq!(out.status, Some(0), "requested /bin/false but the substitute rule should have made curl actually run");
    assert!(out.stdout.contains("curl"), "stdout did not look like curl's own --version banner:\n{}", out.stdout);
}
...
fn coverage_extends_to_a_subprocess_of_a_subprocess() {
    ...
    let plan = ExecConsequencePlan {
        rules: vec![ExecConsequenceRule {
            ...
            match_argv: ArgvMatcher::Argv0 { name: "/nonexistent/touch-canary".to_owned() },
    ...
    // down. The top-level command must be an absolute path (plain execve(),
    // no PATH search — see the passthrough test's note); the nested "sh" can
    // stay bare (it's a real PATH hit, and the outer shell's own PATH search
    // resolves and execve()s it normally, which is itself its own trapped
    // syscall, caught below by passthrough).
    //
    ...
    let out = cell
        .exec(&["stub", "/bin/sh", "-c", "sh -c /nonexistent/touch-canary"], OP_WINDOW)
        .expect("exec");
    assert_eq!(out.status, Some(0));
